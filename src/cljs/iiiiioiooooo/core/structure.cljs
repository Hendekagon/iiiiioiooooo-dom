...
(defn forward-zipper [f] (fn [x] (if (and x (contains? (meta (zip/node x)) :zip/branch?)) (f (zip/node x)) (f x))) )
...
(defn find-first [loc x] (first (filter (comp (partial = x) zip/node) (iterate zip/next loc))))
...
(defn default-state
  ([]
        ...
        (with-meta
         {
          ...
          {
            :esc safe
            :default (fn keyup [s x] (push-history
                        (or
   ...
   ([s h]
    (default-state (push-history (assoc s :context h :focus (zip/up (zip/rightmost (zip/down h))) :selected [(-> h zip/next zip/next)] :qwe 4 :poi "qwe") h) (latest-state h) nil)
   )
   ([h latest w]
    (push-history (assoc (latest-state h) :context (zip/up (find-first h :test2)) :focus (zip/next (zip/next (find-first h :test2))) :qwe "qwe") h)
   )
)
